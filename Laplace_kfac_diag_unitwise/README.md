...
We use the .fit() method to fit the Laplace approximation during SpaM and use it for our marginal likelihood approximation. Note tthatt  we use only forward pass and do not use the predictive approx for our pruning results.
```python
