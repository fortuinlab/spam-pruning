...
class train_cfg():
    ...
    def train(self):
        ...
        def compute_global_pruning_mask(model, scores, sparsity):
            """
            Compute a global pruning mask for the given model based on the provided scores and sparsity level.

            :param model: The model to be pruned.
            :param scores: A dictionary containing pruning scores for each layer.
            :param sparsity: The desired sparsity level (percentage of weights to prune).
            :return: A dictionary containing a binary mask for each layer.
            """
            all_scores = []
            for name, param in model.named_parameters():
                all_scores.append(scores[name].flatten())

            # Combine all scores and sort
            all_scores = torch.cat(all_scores)
            num_to_prune = int(len(all_scores) * sparsity / 100)
            threshold, _ = torch.topk(all_scores, num_to_prune, largest=False, sorted=True)
        ...
        def apply_pruning_mask(model, masks):
            """
            Apply the pruning masks to the model's parameters.

            :param model: The model to be pruned.
            :param masks: A dictionary containing a binary mask for each layer.
            """
            for name, param in model.named_parameters():
                if name in masks:
                    mask = masks[name]
        ...
        prune_scoring = {"SNIP": snip_score}#"SynFlow": synflow_score, "Rand": random_score, "SNIP": snip_score#, "GraSP": grasp_score}
        
        for method in prune_scoring.keys():
            run_name = f"{self.run_name}_PAI_{method}_U_jan"
            with wandb.init(reinit=True, id=wandb.util.generate_id(), config=self.config, project=self.config['wandb']['project'], entity=self.config['wandb']['entity'], name=run_name):
                for sparsity in sparsities:
                    model_copy = copy.deepcopy(self.model)
                    model_copy.to(self.device)
                    scoring = prune_scoring[method](model_copy, self.train_loader, self.config['device'], self.criterion)
                    
                    mask = compute_global_pruning_mask(model_copy, scoring, sparsity)
                    
                    apply_pruning_mask(model_copy, mask)
                    ...
                    wandb.log({"sparsity": check_sparsity(model_copy), "val_acc": val_perf*100,"target_sparsity": sparsity, "marglik": margliks[-1] if margliks else 0})
