...
def SNIP_strategy( model, test_loader, train_loader, sparsities, args):
...
def GraSP_strategy(model, test_loader, train_loader, sparsities,args):
    ...
    we use the same implementation as in the repository above, here i edited the function for other experiments purposes
    """
    criterion = nn.CrossEntropyLoss() 
    models = {}
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    ...
    for sparsity in sparsities:
        ...
        for data, target in train_loader:
            ...
            loss.backward()
            for i, param in enumerate(model_copy.parameters()):
                # use autograd.grad to get the gradient of the loss with respect to the parameter
                #grad_weight = torch.autograd.grad(loss, param, retain_graph=True)[0] # we use this previously for all exp
                grads[i] += param.grad.abs() / len(train_loader) 
...
def Iterative_magnitude_pruning(model, test_loader, train_loader, sparsities, args):
    models = {}
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    model.to(device)
    ...
    for target_sparsity in sparsities:
        ...
        current_sparsity = 0
        step_sparsity_increase = 5  # Increment sparsity by 5% each step to reach target
        
        while current_sparsity < target_sparsity:
            with torch.no_grad():
                flat_params = torch.nn.utils.parameters_to_vector(model_copy.parameters())
                nonzero_params = flat_params[flat_params != 0]
                threshold = torch.quantile(nonzero_params.abs(), step_sparsity_increase / 100)
            ...
            # Fine-tuning
            if args["fine_tune"]:
                print(f"Fine-tuning at sparsity level: {current_sparsity}%")
                la, model_copy, margliks, perf = marglik_optimization(model=model_copy,
                                                                train_loader=train_loader,
                                                                valid_loader=test_loader,
                                                                laplace=KronLaplace,
            ...
            # Logging code, e.g., wandb.log, goes here
