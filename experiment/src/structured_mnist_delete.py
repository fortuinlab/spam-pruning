...
from models import LeNet, CancerNet_fc, MNIST_FC
...
def get_unpruned_neuron_indices(fc_layer):
    ...
    unpruned = np.any(weights != 0, axis=1)  
    return np.nonzero(unpruned)[0]

def copy_weights_fc(structured_zero_model, new_model, unpruned_indices):
    # Get the names of all fully connected layers in the model
    layer_names = [name for name, layer in structured_zero_model.named_modules() if isinstance(layer, nn.Linear)]

    for layer_name in layer_names:
        
        orig_layer = getattr(structured_zero_model, layer_name)
        new_layer = getattr(new_model, layer_name)
        print(f"Orig Layer Weight Shape: {orig_layer.weight.data.shape}")
        print(f"New Layer Weight Shape: {new_layer.weight.data.shape}")
        out_indices = unpruned_indices[layer_name]

        # Calculate the number of input features for this layer
        prev_layer_name = layer_names[layer_names.index(layer_name) - 1] if layer_names.index(layer_name) > 0 else None
        if prev_layer_name:
            prev_layer = getattr(structured_zero_model, prev_layer_name)
            num_input_features = len(unpruned_indices[prev_layer_name])
        else:
            # If it's the first layer, use the indices from the first layer of unpruned_indices
            num_input_features = len(unpruned_indices[layer_names[0]])

        # Adjust input indices to match the number of input features
        in_indices = unpruned_indices[layer_name][:num_input_features]
...
if __name__ == "__main__":
    ...
    dir_zeromod = '/nfs/xxxxxx/pattern/MNIST_FC_mnist_KronLaplace_diagonal_50_wp_struct'
    ...
    for sparsity_level, filename in sparsity_models.items():
        ...
        if not found_matching_model:   
            structured_zero_model = MNIST_FC(784,256,10)
            ...
            print(f"Unpruned indices fc1: {unpruned_indices_fc1}")
            
            copy_weights_fc(structured_zero_model, new_model, {'fc2': unpruned_indices_fc1})
            print(new_model)
            train = True
            if train:
                if "Kron" in filename:
