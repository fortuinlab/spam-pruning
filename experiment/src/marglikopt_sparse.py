...
def linear_sparsity_scheduler(epoch, max_epoch, max_sparsity):
    """Linearly increase sparsity with each epoch."""
    sparsity = (epoch / max_epoch) * max_sparsity
    return min(sparsity, max_sparsity)


def interval_sparsity_scheduler(epoch, max_epochs, initial_sparsity, final_sparsity, update_interval):
    sparsity_progress = (epoch // update_interval) / (max_epochs // update_interval)
    sparsity = initial_sparsity + (final_sparsity - initial_sparsity) * sparsity_progress
    
    sparsity = max(min(sparsity, final_sparsity), initial_sparsity)

    sparsify_now = (epoch % update_interval == 0) or (epoch == max_epochs - 1)

    return sparsity, sparsify_now


def apply_sparsity(model, posterior_precision, target_sparsity, sp_method="post_and_weights"):
    """
    Apply sparsity to the model weights based on the sparsity_level passed (stepwise).
    This function zeros out the smallest weights**2 * post or post to achieve the given sparsity level.
    """
    flat_precisions = posterior_precision.flatten()
    
    if sp_method == "post_only":
        sorted_indices = flat_precisions.argsort()

        # Calculate the cutoff for sparsity
        cutoff_index = int(target_sparsity * len(flat_precisions))
        threshold = flat_precisions[sorted_indices[cutoff_index]]

        # Zero out weights with precision below the threshold
        for p in model.parameters():
            precision_mask = posterior_precision[:p.numel()].reshape(p.shape) >= threshold
            p.data = p.data * precision_mask.to(p.data.device)
            posterior_precision = posterior_precision[p.numel():]
    elif sp_method == "post_and_weights":
        weights = parameters_to_vector(model.parameters())
        weig_mult = weights.flatten()
        importance = (weig_mult**2) * flat_precisions
        sorted_indices = importance.argsort()

        cutoff_index = int(target_sparsity * len(importance))
        threshold = importance[sorted_indices[cutoff_index]]
        
        for p in model.parameters():
            precision_mask = importance[:p.numel()].reshape(p.shape) >= threshold
            p.data = p.data * precision_mask.to(p.data.device)
            importance = importance[p.numel():]
    else:
        print("Invalid sparsity method"	)
...
def marglik_optimization(model,
    ...
    max_sparsity = 0.99  # for example, you want to prune 90% of the weights
    max_epoch = n_epochs
    ...
    for epoch in range(1, n_epochs + 1):
        ...
        #                temperature=temperature, backend=backend)
        #lap.fit(train_loader)
        if isinstance(lap, DiagLaplace):
            posterior_precision = lap.posterior_precision
        else:
            posterior_precision  = lap.posterior_precision.diag()
        target_sparsity_epoch, sparsify_now = interval_sparsity_scheduler(epoch, n_epochs, 0.1, target_sparsity, update_interval)
        #target_sparsity = linear_sparsity_scheduler(epoch, max_epoch, max_sparsity)
        if sparsify_now:
            apply_sparsity(model, posterior_precision, target_sparsity_epoch, sp_method=sparse_method)
            current_sparsity = calculate_sparsity(model)
            # the relogging of val acc is to be able to extract and compare the other data from the wandb log
            # the name target in this dictionary is as well to be able to align the x axis with previous plots
            epoch_log.update({'sparsity': current_sparsity,"target_sparsity":target_sparsity_epoch*100} )
            if log_wandb == True:
                wandb.log(epoch_log)
            # Verbosity
            if verbose:
                print(f"Epoch {epoch}/{n_epochs}")
                print(f"Target Sparsity: {target_sparsity_epoch:.2%}")
                print(f"Actual Sparsity: {current_sparsity:.2%}")
                print(f"Epoch Loss: {epoch_loss:.4f}")

        else:
            print(f"Epoch {epoch}/{n_epochs}, Sparsity not updated")
        ...
        if valid_loader is not None:
            with torch.no_grad():
                ...
                epoch_log.update({'valid/perf': val_perf, 'valid/nll': val_nll,'val_acc': val_perf*100})
                if log_wandb == True:
                    wandb.log(epoch_log)
