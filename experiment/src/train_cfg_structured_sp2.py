...
class train_cfg():
    ...
    def train(self):
        ...
        prune_scoring = {"SynFlow": synflow_score, "Rand": random_score, "SNIP": snip_score, "GraSP": grasp_score}#,"Laplace":None}
        ...
        for method in prune_scoring:
            if  method != "Laplace":
                run_name = f"{self.run_name}_structured_pruning_wp_maskaftereach_{method}_skipoutput"
                with wandb.init(reinit=True, id=wandb.util.generate_id(), config=cfgs, project=self.config['wandb']['project'], entity=self.config['wandb']['entity'], name=run_name):      
                    ...
                    for sparsity in sparsities:
                        ...
                        wandb.log({"sparsity": sparsity, "val_acc": val_acc, "marglik": margliks[-1], "total_neurons_pruned": neurons_pruned, "total_filters_pruned": filters_pruned})
                        #if self.infere_exp == True:
                        #    model_name = self.path_infere_cfg + f"{self.run_name}_acc_{val_acc}_sparsity_{sp}_{method}_maskaftereach_nn_{method}.pt"
