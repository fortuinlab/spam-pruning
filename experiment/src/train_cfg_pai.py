...
from sparsify_v2_pai import (sparse_strategy, magnitude_pruning, random_strategy, 
...
class train_cfg():
            ...
            if laplace_type is not None:
                if sparse_method is not None:
                    ...
                    if self.use_map or self.config['laplace']['n_epochs_burnin']> epoch :
                        self.use_map = True
                        return f'{model_name}_{self.config["dataset_name"]}_{epoch}_PAI_MAP_brier_new_fixed'
                    else:
                        if self.resnet_inplanes == 64 and self.config['model_name'] == 'ResNet':
                            return f'{model_name}_64_aug_{self.config["dataset_name"]}_PAI_{laplace_type}_{prior}_{epoch}_wp'
                        else:
                            return f'{model_name}_{self.config["dataset_name"]}_PAI_brier_new_fixed_EF_{laplace_type}_{prior}_{epoch}_wp'
    ...
    def train(self):
        ...
        self.save_path_inference = Path(__file__).parent.parent.parent.parent.parent.parent / "students/xxxxx/inference/"
        ...
        if self.config['train_marglik']:
            ...
            if margopt:
                ...
                args_sparse= {
                    ...
                    'prior_structure': self.config['laplace']['prior_structure'],
                    'tune_epochs_burnin': self.config["num_epochs"]+1,
                    'marglik_frequency': self.config['laplace']['marglik_frequency'],
                    'fine_tune': self.config["tune"],
                    'tune_epochs': self.config["num_epochs"],
                    'lr': self.config['lr'],
                    "lr_min":self.config['laplace']['lr_min'],
                    "optimizer":self.config['optimizer'],
                    "temperature": self.temperature,
                ...
                sparse_list = {#'laplacekron':{'model': copy.deepcopy(model), 'function': sparse_strategy ,'sparsities': sparsities, 'la': la},
                                ...
                                'SNIP':{'model': copy.deepcopy(model), 'function': SNIP_strategy ,'sparsities': sparsities},
                                'GraSP':{'model': copy.deepcopy(model), 'function': GraSP_strategy ,'sparsities': sparsities},
                                }
                ...
                if self.sparse_exact_same == True:
                    ...
                    for sparse_name, sparse_dict in sparse_list.items():
                        cfgs = self.config
                        run_name = f"{self.run_name}_sub_{sparse_name}" if args_sparse['fine_tune'] == True else f"{self.run_name}_sub_{sparse_name}"
                        ...
                        with wandb.init(reinit=True, id=wandb.util.generate_id(),config = cfgs, project=self.config['wandb']['project'], entity=self.config['wandb']['entity'], name=run_name):
                            ...
                            models_stats = sparse_dict['function'](model = sparse_dict['model'], test_loader= self.valid_loader,
